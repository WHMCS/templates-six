...
class lessc {
	...
	protected function tryImport($importPath, $parentBlock, $out) {
		...
		$url = $this->compileValue($this->lib_es($str));
	...
	protected function lib_esxtract($value) {
	...
	protected function lib_es($arg) {
		switch ($arg[0]) {
			case "list":
				$items = $arg[2];
				if (isset($items[0])) {
					return $this->lib_es($items[0]);
				}
	...
	protected function lib__sprintf($args) {
		...
		$template = $this->compileValue($this->lib_es($string));
		...
		if (preg_match_all('/%[dsa]/', $template, $m)) {
			foreach ($m[0] as $match) {
				...
				$i++;
				$rep = $this->compileValue($this->lib_es($val));
	...
	protected function lib_unit($arg) {
		if ($arg[0] == "list") {
			...
			return array("number", $this->assertNumber($number),
				$this->compileValue($this->lib_es($newUnit)));
		} else {
	...
	protected function reduce($value, $forExpression = false) {
		...
		case "interpolate":
			...
			if (empty($value[2])) $res = $this->lib_es($res);
		...
		case "variable":
			...
			if (is_array($key)) {
				$key = $this->reduce($key);
				$key = $this->vPrefix . $this->compileValue($this->lib_es($key));
			}
		...
		case "string":
			foreach ($value[2] as &$part) {
				if (is_array($part)) {
					...
					if ($strip) $part = $this->lib_es($part);
				}
			}
			return $value;
		case "escape":
			list(,$inner) = $value;
			return $this->lib_es($this->reduce($inner));
		case "function":
